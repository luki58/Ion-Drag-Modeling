...
def model_khrapak(gas_type, I, polarity, E_multiplier, ne_multiplier, Te_multiplier, z_multiplier, e=e):
    ...
    if gas_type == "Neon":
        ...
        if theory == 2:
            z = [ .59, .52, .4, .4, .4, .4, .4, .4, .37, .36, .34, .32]  # Charge potential adjsutable 0.3 +- 0.1 NEON, 0.4 +-1 ARGON; Antonova et. al.
        else:
    ...
    if gas_type == "Argon" and I == 1.5:
        n_d = np.array([0.08, 0.1, 0.15, 0.2, 0.25, 0.3, 0.4, 0.4, 0.4, 0.6, 0.7, 0.8]) * 10**11  #? Dust number density in m^-3; not sure about this value
    ...
    E_0_calc = [e_field(p[0], I), e_field(p[1], I), e_field(p[2], I), e_field(p[3], I), e_field(p[4], I), e_field(p[5], I), e_field(p[6], I), e_field(p[7], I), e_field(p[8], I), e_field(p[9], I), e_field(p[10], I), e_field(p[11], I)]
    E_0 = np.multiply(E_0_calc, -100*E_multiplier)  # V/m
    T_e = np.array([T_e_interpolation(p[0], I),T_e_interpolation(p[1], I),T_e_interpolation(p[2], I), T_e_interpolation(p[3], I), T_e_interpolation(p[4], I), T_e_interpolation(p[5], I), T_e_interpolation(p[6], I), T_e_interpolation(p[7], I), T_e_interpolation(p[8], I), T_e_interpolation(p[9], I), T_e_interpolation(p[10], I), T_e_interpolation(p[11], I)])
    n_e0 = np.multiply([n_e_interpolation(p[0], I),n_e_interpolation(p[1], I), n_e_interpolation(p[2], I), n_e_interpolation(p[3], I), n_e_interpolation(p[4], I), n_e_interpolation(p[5], I), n_e_interpolation(p[6], I), n_e_interpolation(p[7], I), n_e_interpolation(p[8], I), n_e_interpolation(p[9], I), n_e_interpolation(p[10], I), n_e_interpolation(p[11], I)], ne_multiplier * 10**14)
    
    if gas_type == "Argon":
        T_e_argon = T_e_argon * Te_multiplier * T_e_argon_neon_translation
    else:
    ...
    if gas_type == "Neon":
        ...
        T_i = (2 / 9) * np.abs(E_0) * e / k * l_i + T_n * eV_K
    else:
        l_i = np.divide(T_n  * eV_K * k, p * sigma_argon)
        T_i = (2 / 9) * np.abs(E_0_argon) * e / k * l_i + T_n * eV_K
    ...
    if gas_type == "Neon":
        ...
        v_ti = np.sqrt(8 * k * T_n * eV_K / (np.pi * m_argon)) 
    ...
    if gas_type == "Neon":
        ...
        nue = np.sqrt((8*T_n*k*eV_K/(np.pi*m_neon)) + u_i**2 * (1+ ((u_i/v_boom)/(0.6 + 0.05*np.log(m_neon) + (debye_De/(5*a))*(np.sqrt(T_i/T_e)-.1)))**3))
        ...
        nue = np.sqrt((8*T_n*k*eV_K/(np.pi*m_argon)) + u_i**2 * (1+ ((u_i/v_boom)/(0.6 + 0.05*np.log(m_argon) + (debye_De/(5*a))*(np.sqrt(T_i/T_e_argon)-.1)))**3))
    ...
    beta_T3 = roh_0_nue/debye_nue
    
    # Integration for ion drag force
    def integration_function(x, debye_Di_val, beta_val):
        #return np.log(debye_Di_val/roh_0_val)
        return 2 * np.exp(-x) * np.log((2 * x + beta_val) / (2 * a * x / debye_Di_val + beta_val))
    ...
    integrated_f = np.array([
        integrate(lambda x: integration_function(x, debye_D[i], beta_T[i]), 0, np.inf)[0]
        for i in range(len(p))
    ])
